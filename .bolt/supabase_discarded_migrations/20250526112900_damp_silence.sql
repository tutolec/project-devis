/*
  ...
  2. Corrections
    ...
    - Ajout de la colonne vmc_power avec ses contraintes
*/
...
BEGIN
  ...
  -- Ajout de la nouvelle colonne vmc_power
  IF NOT column_exists('construction_forms', 'vmc_power') THEN
    ALTER TABLE construction_forms ADD COLUMN vmc_power text;
  END IF;
  ...
  ALTER TABLE construction_forms 
    ...
    ALTER COLUMN vmc_power SET NOT NULL;
  ...
  -- Ajout de la contrainte pour vmc_power
  IF NOT EXISTS (
    SELECT 1 
    FROM information_schema.table_constraints 
    WHERE constraint_name = 'vmc_power_values'
  ) THEN
    ALTER TABLE construction_forms
    ADD CONSTRAINT vmc_power_values 
    CHECK (vmc_power IN ('yes', 'yes-switch', 'no', 'unknown'));
  END IF;
